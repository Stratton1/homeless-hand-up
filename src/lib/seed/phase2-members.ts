...
export interface CommunityMember {
  /** Unique identifier */
  id: string;
  /** Primary key in Supabase when available */
  databaseId?: string;
  ...
  balancePence: number;
  /** Spendable balance available at approved retailers (pence) */
  spendableBalancePence?: number;
 ...
 */
export const phase2CommunityMembers: CommunityMember[] = [
  {
    id: "usr_001",
...
export function getPlatformStats(): PlatformStats {
  const members = phase2CommunityMembers.filter((m) => m.active);
...
export function getAllLocations(): string[] {
  const locations = phase2CommunityMembers.filter((u) => u.active).map((u) => u.location);
